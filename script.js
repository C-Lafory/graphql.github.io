...
function setGraph(tab) {
    ...
    for (let i = 0; i < 17; i++) {
        ...
        if (first_date.getMonth() + i > 12) {
            axeXContent += `<text x="${axeX}" y="620" font-family="Verdana" font-size="15" fill="black">${first_date.getMonth() + i - 11}/${(first_date.getFullYear() + 1) % 100}</text>`;
        } else {
            axeXContent += `<text x="${axeX}" y="620" font-family="Verdana" font-size="15" fill="black">${first_date.getMonth() + i + 1}/${first_date.getFullYear() % 100}</text>`;
        }
    }
    let points = "";
